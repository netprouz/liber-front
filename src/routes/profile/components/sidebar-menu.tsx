...
import { Divider, ListItemIcon, MenuItem, MenuList, Paper, Switch, Typography } from "@mui/material";
...
const ProfileSidebarMenu: React.FC<Settings> = ({
  ...
  return (
    <Paper sx={theme=>({
      width: 300,[theme.breakpoints.down("md")]:{
        width:"100%"
      }
    })}>
      <MenuList>
        {Menu.map((item, i) => {
          return <MenuItem  key={i} disableGutters={true} onClick={() => setActiveItemId(item.id)}>
            {activeItemId === item.id && <Divider orientation="vertical" flexItem sx={{
              background: "#3F51B5",
              width: 2
            }} />}
            <MenuWithICon>
              ...
              {item.id === 6 && <Switch checked={isChecked} sx={{ marginLeft: 5 }}
              />}
